...
| LiveKit crash-loops with `panic: interface conversion: *sfu.RedPrimaryReceiver is not sfu.REDTransformer: missing method HasDownTracks` (server dies when the device/agent joins) | known livekit-server v1.9.10 regression (upstream livekit#4244), fixed in v1.9.11 (#4219/#4220) — keep the `livekit/livekit-server` image at v1.9.11+ in `docker-compose.yml` and `docker compose up -d livekit`. The agent-side `Connection refused` bursts above are just this restart window, not a client (ESP32) fault |
