...
- Outbound internet for: Docker Hub, PyPI (first build), ElevenLabs, OpenAI,
  Open-Meteo, and Hugging Face (one-time turn-detector model download)
...
```bash
docker compose up -d --build     # livekit + weather-mcp + agent
docker compose ps
```
...
(cloud VM) or `rtc.node_ip: <host-LAN-IP>` (LAN), and point
`LIVEKIT_URL`/`WEATHER_MCP_URL` at the service names instead of `localhost`.
...
| Weather answers "no default location" | set `DEFAULT_LOCATION` and `docker compose up -d --force-recreate agent` |
